...
- **REQ-SAFE-002 [ ]** Only the human speaks with the human's authority. Content from files, web pages, tool output and
...
## Kill switch (#42)
- **REQ-SAFE-010 [ ]** **Stop everything** is available as:
...
- **REQ-SAFE-020 [ ]** `[safety] protected` in `troupe.toml` lists paths (files or directories) that agents can't change
...
- **REQ-SAFE-030 [ ]** Never expose secrets.
...
- **REQ-SAFE-031 [ ]** Secret scan on every commit troupe makes: task commits (REQ-ENG-033), main autocommits
...
- **REQ-SAFE-033 [ ]** No secrets in outbound requests: web fetch/search calls and shell commands (curl, wget, http
...
- **REQ-SAFE-034 [ ]** Enforcement per backend:
  ...
  - codex: its sandbox and exec policy apply them where the CLI allows (REQ-SAFE-050). Every gap is listed in the ADR.
...
## Audit (#42)
- **REQ-SAFE-040 [ ]** Every safety event is recorded: blocked actions, kill-switch use, resume, approval-card outcomes,
